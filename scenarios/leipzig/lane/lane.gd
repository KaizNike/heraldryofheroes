...
func _ready():
	...
	Data.get_node("Party").location = "res://scenarios/leipzig/lane/lane.tscn"
	Data.save_game()
...
func fade_in_scenery():
	...
	if winter:
		if night:
			texture = load("res://scenarios/leipzig/lane/Lane-WN.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/lane/Lane-WD.jpg")
			$FadePanel/Player.play("fade_in_light")
	else:
		if night:
			texture = load("res://scenarios/leipzig/lane/Lane-SN.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/lane/Lane-SD.jpg")
			$FadePanel/Player.play("fade_in_light")
