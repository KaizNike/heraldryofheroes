...
func _ready():
	...
	Data.get_node("Party").dict["location"] = "res://scenarios/leipzig/metal_st/metal_st.tscn"
	Data.save_game()
	
	Main.get_node("Audio/SFX").stream = load("res://audio/sfx/smithy.ogg")
...
func fade_in_scenery():
	...
	if winter:
		if night:
			texture = load("res://scenarios/leipzig/metal_st/metal_st_sd.jpg")
