...
func _ready():
	...
	Data.get_node("Party").dict["location"] = "res://scenarios/leipzig/garden/garden.tscn"
	Data.save_game()
...
func fade_in_scenery():
	...
	if winter:
		if night:
			texture = load("res://scenarios/leipzig/garden/garden_wn.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/garden/garden_wd.jpg")
			$FadePanel/Player.play("fade_in_light")
	else:
		if night:
			texture = load("res://scenarios/leipzig/garden/garden_sn.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/garden/garden_sd.jpg")
			$FadePanel/Player.play("fade_in_light")
