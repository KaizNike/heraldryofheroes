...
func _ready():
	...
	Data.get_node("Party").dict["location"] = "res://scenarios/leipzig/church/church.tscn"
	Data.save_game()
...
func fade_in_scenery():
	...
	if winter:
		if night:
			texture = load("res://scenarios/leipzig/church/church_wn.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/church/church_wd.jpg")
			$FadePanel/Player.play("fade_in_light")
	else:
		if night:
			texture = load("res://scenarios/leipzig/church/church_sn.jpg")
			$FadePanel/Player.play("fade_in_dark")
		else:
			texture = load("res://scenarios/leipzig/church/church_sd.jpg")
			$FadePanel/Player.play("fade_in_light")
